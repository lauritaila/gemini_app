          ...
          CustomBottomInput(
            onSend: (partialText, {List<XFile> images = const []}) {
              ...
              if (selectedArtStyle.isNotEmpty) {
                promptWithStyle += ', en estilo de $selectedArtStyle';
              }
              String prompt = promptWithStyle;
...
class GeneratedImageGallery extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return SizedBox(
      ...
      child: PageView(
        onPageChanged: (index){
          if(index == generatedImages.length - 1 && isGenerating){
            ref.read(generatedImagesProvider.notifier).generateImageWithPreviousPrompt();
          }
        },
